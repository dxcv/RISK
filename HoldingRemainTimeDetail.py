...
class HoldingDetail:
    def __init__(self, fundcode, fdate=None):
        ...
        self.MSSQLOP_71 = DataOP.MSSQLOP('bi', '71@LHYYZX', '172.16.100.71', 'bi')
        self.Sqlalchemyconn = create_engine('mssql+pymssql://bi:71@LHYYZX@172.16.100.71:1433/bi?charset=utf8')
    ...
    def GetHoldingDetail(self):
        ...
        df['Rank'] = df[['rank_1year', 'rank_3year', 'IS_SCP']].apply(
            lambda x: x['rank_3year'] if x['IS_SCP'] else x['rank_1year'], axis=1)

        # 是否利率债
        logging.info('判断是否为利率债！')
        if df[df['TYPE'] == '债券'].shape[0] != 0:
            df['IS_Rate_Bond'] = np.logical_or(
                np.logical_or(df['S_INFO_INDUSTRYNAME'] == '国债', df['S_INFO_INDUSTRYNAME'] == '央行票据'),
                np.logical_and(df['S_INFO_INDUSTRYNAME'] == '金融债', df['S_INFO_INDUSTRYNAME2'] == '政策银行债'))
        else:
    ...
    def SaveHoindingDetail(self):
        ...
        logging.info("存入数据库！")
        self.MSSQLOP_71.delete_data(DeleteHoldingDetailSQL % (self.fdate, self.fundcode))

        pd.io.sql.to_sql(self.holdiingdetail, 'HoldingDetail', con=self.Sqlalchemyconn, if_exists='append', index=False)
...
dict = {'鑫盈': '000439', '及第7天': '003002'}
for value in dict.values():
    Ho = HoldingDetail(value, '2018-08-15')  # 可选参数 fdate 格式 ’2018-07-17‘
